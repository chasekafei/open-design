...
export async function startServer({
  ...
  if (isApiTokenMiddlewareEnabled()) {
    ...
    // Authorization. The cross-origin middleware accepts `Origin: null`
    // for /projects/:id/raw/...; mirror that exception here so the
    // bearer check does not 401 the iframe before the cross-origin
    // check has a chance.
